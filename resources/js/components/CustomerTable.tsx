...
export default function CustomerTable({ customers, teamSlug, onEdit }: Props) {
    function handleDelete(customer: Customer) {
        ...
        router.delete(`/${teamSlug}/customers/${customer.id}`, {
            preserveScroll: true,
        });
    }
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-border-soft">
                        <th className="px-6 py-3.5 text-left text-[13px] font-medium text-text-secondary">Name</th>
                        <th className="px-6 py-3.5 text-left text-[13px] font-medium text-text-secondary">Phone</th>
                        <th className="px-6 py-3.5 text-left text-[13px] font-medium text-text-secondary">Email</th>
                        <th className="px-6 py-3.5 text-left text-[13px] font-medium text-text-secondary">Loyalty Points</th>
                ...
                <tbody>
                    {customers.map((customer, idx) => (
                        <tr
                            key={customer.id}
                            className={idx !== customers.length - 1 ? 'border-b border-border-soft' : ''}
                        >
                            ...
                            <td className="px-6 py-4 text-text-secondary">
                                {customer.email ?? <span className="italic text-text-secondary">—</span>}
                            </td>
