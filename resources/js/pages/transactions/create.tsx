    ...
    return (
        <>
            ...
            <div className="flex h-full gap-6 p-6">
                ...
                {/* Right — Cart */}
                <div className="flex w-80 flex-col gap-4 rounded-xl border border-border-soft bg-surface p-5">
                        ...
                        {cart.map(item => (
                            <div key={item.product.id} className="flex items-center gap-2 rounded-lg border border-border-soft p-2">
                                <div className="flex-1">
                                    <p className="text-xs font-medium text-text-primary">{item.product.name}</p>
...
TransactionCreate.layout = (props: { currentTeam?: { slug: string } | null }) => ({
    breadcrumbs: [
        {
            title: 'Transactions',
